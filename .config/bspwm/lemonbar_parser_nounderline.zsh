...
while read -r line ; do
  case $line in
    SYS*)
      ...
      if [[ "${cpu_percent}" -gt 20 ]]; then
          ...
          cpu_color=${c_blue_foreground}
      fi
      cpu="%{+u U${c_lightgray} T4 F${c_white}} ${icon_cpu}%{F-} %{F${cpu_color} T1}$(printf '%+3s' $sys_arr[(w)1]% {F-}%)%{-u U-}"

      # mem
      mem="%{+u U${c_graygreen} T4 F${c_white}} ${icon_memory}%{F${c_blue_foreground}} %{T1}$(printf '%+5s' $sys_arr[(w)2])%{-u U-}"
      ...
      net="%{+u U${c_lightgreen} A:'/home/daniel/.config/bspwm/startnm.sh':} ${net_icon}${stab}%{T2}${net_down_icon} %{T1 F${c_blue_foreground}}$(printf '%5s' $wland)  %{T2}${net_up_icon} %{T1 F${c_blue_foreground}}$(printf '%5s' $wlanu)%{-u U- A}"
      ...
      weather="%{+u U${c_mediumblue} T1 A:/home/daniel/.config/bspwm/startweather.zsh: F${temp_col}}${weather_temp}%{F-}F%{-u U- A}"

      # date
      date="%{+u U${c_lightgray} T2 A:/home/daniel/.config/bspwm/startcalendar.zsh: F${c_white}} %{F- T1} $sys_arr[(w)7] $sys_arr[(w)8] $sys_arr[(w)9] $sys_arr[(w)10]%{-u U- A}"

      # time
      time="%{+u U${c_orange} T2 A:/home/daniel/.config/bspwm/startcalendar.zsh: F${c_white}}%{F- T1}$sys_arr[(w)11]%{-u U- A}"
      ;;
    ...
    VOL*)
      ...
      vol="%{+u U${c_lightred} A:/home/daniel/.config/bspwm/startpavucontrol.zsh:} ${vol_icon}%{T1 F${c_blue_foreground}} $(printf '%+4s' $vol_txt%)%{-u U- A F-}"
      ;;
    
    THR*)
      thermal_arr=(${line#???})
      thermal="%{+u U${c_orange} T5} %{F$c_white}${icon_thermal}%{F${c_blue_foreground}}%{T1}$thermal_arr%{-u U- F-}"
      ;;
    ...
    BRI*)
      ...
      if [[ $brightness_actual -eq 0 ]]; then
          ...
          brightness="%{U${c_lightgreen} +u T2} %{F${c_white}}${icon_brightness} %{T1 F${c_blue_foreground}}$(printf '%3s' $brightness_arr)%%{-u U-} "
      fi
      ;;
      ...
      foreach desktop ({2..$n_loop})
          ...
          case $current_desktop in
             O*)
                 ...
                 wsp_output="${wsp_output}%{+u U${c_orange} F${c_white}} ${current_desktop:1:1} %{-u U- B-}"
                ;;
             F*)
                 wsp_output="${wsp_output}%{+u U${c_orange} F${c_white}} ${current_desktop:1:1} %{-u U- B-}"
                ;;
             o*)
                 #icon_string="$(. $(dirname $0)/get_icons.zsh $desktop_list[(w)$[$desktop-1]])"
                 wsp_output="${wsp_output}%{+u U${c_lightgray} F${c_blue_foreground} A:bspc desktop -f ${current_desktop:1:1}:} ${current_desktop:1:1} %{-u F- U- A}"           
                ;;
          esac
      end

      #wsp_output="${wsp_output}%{F-}"
    ...
    TITLE*)
        title="%{F${c_green}}$(echo ${line#?????} | cut -c 1-75)%{F-}"
      ;;
  ...
  # And finally, output
  printf "%s\n" "%{l}${wsp_output}${stab}${title}%{c}${date}${stab}${time}${stab}${weather}%{r} ${lock_string}${stab}%{A:zsh /home/daniel/.config/bspwm/startksysguard.zsh:}${cpu}${stab}${thermal}${stab}${mem}${stab}%{A}${bat}${stab}${vol}${stab}${brightness}${net}${stab}${shutdown}${stab} "
done
